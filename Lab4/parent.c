...
void producer_loop(void) {
    ...
    while (!needTerminate) {
        ...
        if (sem_op(semid, SEM_EMPTY_COUNT, -1) == -1) {
        ...
        if (sem_op(semid, SEM_FILL_COUNT, 1) == -1) {
            break;
        }
...
int main(void) {
    ...
    if (sem_setval(semid, SEM_FILL_COUNT, 0) < 0) {
        perror("semctl(fillcount)");
        exit(1);
    }
    if (sem_setval(semid, SEM_EMPTY_COUNT, QUEUE_SIZE) < 0) {
